...
Oracul::Oracul(int M, int N) : M(M), N(N) {
...
double Oracul::clip_x(double x) {
    return std::max(X_left, std::min(X_right, x));
}

double Oracul::clip_y(double y) {
    return std::max(Y_down, std::min(Y_up, y));
}

double Oracul::get_a_xy(double y1, double y2, double x) {
    if (y1 > -4*x/3 + 4) {
        return 1 / eps;
    }
    if (y2 <= -4*x/3 + 4) {
...
double Oracul::get_a(int i, int j) {
...
double Oracul::get_b_xy(double y, double x1, double x2) {
    if (x1 > -3*y/4 + 3) {
        return 1 / eps;
    }
    if (x2 <= -3*y/4 + 3) {
...
double Oracul::get_b(int i, int j) {
...
int Oracul::is_inside_area(double x, double y) {
...
double Oracul::get_square(double x1, double x2, double y1, double y2) {
    if (3*y1 + 4*x1 > 12) {
        return 0;
...
const std::vector<std::vector<double>> Oracul::getAw(const std::vector<std::vector<double>>& w) {
...
const std::vector<std::vector<double>> Oracul::getB() {
    return B_matrix;
}

void Oracul::border_conditions(std::vector<std::vector<double>>& w) {
    int i, j;
